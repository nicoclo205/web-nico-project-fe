							...
							<div className="flex items-center justify-between mb-1">
								...
								<div className="flex items-center gap-1">
									...
									<span className="text-xs text-gray-400">
										{formatDate(match.fecha_partido)}
									</span>
								</div>
							</div>
						...
						<div className="divide-y divide-white/5">
							{match.predictions.length === 0 ? (
								<p className="text-xs text-gray-500 text-center py-3">
									{t('rooms:predictions.noPredictions')}
								</p>
							) : (
										...
										<div className="flex items-center gap-3">
											{p.is_knockout && p.tiene_penales ? (
												<span
													...
													title={
														p.ganador_ko_nombre
															? `Penalties - winner: ${p.ganador_ko_nombre}`
															: 'Penalties'
													}
												>
													Penalty Kicks
											...
											{isFinished && (
												<span
													className={`text-xs min-w-[40px] text-right ${getBetStyle(p.estado)}`}
												>
													{p.estado === 'ganada'
														? `+${p.puntos_ganados} pts`
														: '0 pts'}
												</span>
											)}
										</div>
